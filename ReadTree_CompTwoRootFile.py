...
print "total_number_of_plots: ",total_number_of_plots
print "branchesToPlot.keys(): ",branchesToPlot.keys()

# VarsToImport = [k.split('/') for k in branchesToPlot.keys()]

VarsToImport = []
for k in branchesToPlot.keys():
    if "/" in k:
        for vars_ in k.split('/'):
            VarsToImport.append(vars_)
    else:
        VarsToImport.append(k)

print "VarsToImport: ",VarsToImport
...
for count,files in enumerate(input_root_file_list):
    ...
    branches.append(trees[count].arrays(VarsToImport))
...
for count,var_plots in enumerate(sorted(branchesToPlot)):
    ...
    for fileCount in range(0, len(input_root_file_list)):
        if "/" in var_plots:
            n, bins, patches = plt.hist(branches[fileCount][var_plots.split("/")[0]]/branches[fileCount][var_plots.split("/")[1]],
                    bins='auto',
    ...
    # plt.show()
    plt.savefig(args.dir_to_save_plots+os.sep+var_plots.replace('/','_')+'.png')
    plt.savefig(args.dir_to_save_plots+os.sep+var_plots.replace('/','_')+'.pdf')
    if (args.logY):
        plt.yscale('log')
        plt.savefig(args.dir_to_save_plots+os.sep+var_plots.replace('/','_')+'_log.png')
        plt.savefig(args.dir_to_save_plots+os.sep+var_plots.replace('/','_')+'_log.pdf')
        plt.yscale('linear')
    plt.close()
"""
"""
