...
void plotEfficiency_ZPrime()
{
    ...
    t1->Draw("eg_trkIsol>>h1_trkIso","abs(eg_eta)<1.47");
    t2->Draw("eg_trkIsol>>h2_trkIso","abs(eg_eta)<1.47");
    ...
    t1->Draw("eg_et>>h1_num","eg_trkIsol<1.0 && abs(eg_eta)<1.47");
    t1->Draw("eg_et>>h1_den","abs(eg_eta)<1.47");
    t2->Draw("eg_et>>h2_num","eg_trkIsol<1.0 && abs(eg_eta)<1.47");
    t2->Draw("eg_et>>h2_den","abs(eg_eta)<1.47");
    ...
    gPad->Update();
    auto graph = pEff_sig_PataTrk->GetPaintedGraph();
    // graph->SetMinimum(0.0);
    //    graph->SetMaximum(1.05);
    // graph->SetMaximum();
    c2->Update();
